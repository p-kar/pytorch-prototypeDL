...
model_folder = os.path.join(os.getcwd(), "saved_model_inter", "mnist_model", "mnist_cae_1")
...
filter_reverse_4 = [f_4,f_4,n_map_4,n_map_3]
filter_reverse_3 = [f_3, f_3, n_map_3, n_map_2]
filter_reverse_2 = [f_2, f_2, n_map_2, n_map_1]
filter_reverse_1 = [f_1, f_1, n_map_1, n_input_channel]
...
weights = {
    ...
    'enc_f4': tf.Variable(tf.random_normal(filter_shape_4,
                                           ...
                                           dtype=tf.float32),
                          name='encoder_f4'),
    'dec_f4': tf.Variable(tf.random_normal(filter_reverse_4,
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='decoder_f4'),
    'dec_f3': tf.Variable(tf.random_normal(filter_reverse_3,
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='decoder_f3'),
    'dec_f2': tf.Variable(tf.random_normal(filter_reverse_2,
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='decoder_f2'),
    'dec_f1': tf.Variable(tf.random_normal(filter_reverse_1,
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='decoder_f1')
}
...
def upsample(input, filter, bias, output_shape, strides, padding="VALID", nonlinearity=tf.nn.relu):
    print (output_shape)
    shape_size = output_shape[1:3]
    inter = tf.image.resize_bilinear(input,shape_size)
    conv = tf.nn.conv2d(inter, filter, strides=[1,1,1,1], padding=padding)
    act = nonlinearity(conv + bias)
    return act
...
dl4 = upsample(reshape_feature_vectors, weights['dec_f4'], biases['dec_b4'],
                   output_shape=[deconv_batch_size, l3_shape[1], l3_shape[2], l3_shape[3]],
                   strides=stride_4, padding="SAME", nonlinearity=tf.nn.sigmoid)
dl3 = upsample(dl4, weights['dec_f3'], biases['dec_b3'],
                   output_shape=[deconv_batch_size, l2_shape[1], l2_shape[2], l2_shape[3]],
                   strides=stride_3, padding="SAME", nonlinearity=tf.nn.sigmoid)
dl2 = upsample(dl3, weights['dec_f2'], biases['dec_b2'],
                   output_shape=[deconv_batch_size, l1_shape[1], l1_shape[2], l1_shape[3]],
                   strides=stride_2, padding="SAME", nonlinearity=tf.nn.sigmoid)
dl1 = upsample(dl2, weights['dec_f1'], biases['dec_b1'],
...
with tf.Session(config=config) as sess:
    ...
    n_train_batch = len(train_images) //batch_size #mnist.train.num_examples // batch_size
    n_valid_batch = len(test_images)//batch_size #mnist.validation.num_examples // batch_size
    n_test_batch = len(test_images)//batch_size #mnist.test.num_examples // batch_size
    # Training cycle
    for epoch in range(training_epochs):
        ...
        for i in range(n_train_batch):
            batch_x, batch_y = train_images[i*250:i*250+250],train_labels[i*250:i*250+250]#mnist.train.next_batch(batch_size)
        ...
        for i in range(n_valid_batch):
            batch_x, batch_y = test_images[i*250:i*250+250],test_labels[i*250:i*250+250]#mnist.validation.next_batch(batch_size)
            batch_x = np.reshape(batch_x,(batch_x.shape[0],batch_x.shape[1]*batch_x.shape[2]))
            batch_y = to_categorical(batch_y)
        ...
        if epoch % test_display_step == 0 or epoch == training_epochs - 1:
            ...
            for i in range(n_test_batch):
                batch_x, batch_y = test_images[i*250:i*250+250],test_labels[i*250:i*250+250]#mnist.test.next_batch(batch_size)
                batch_x = np.reshape(batch_x,(batch_x.shape[0],batch_x.shape[1]*batch_x.shape[2]))
                batch_y = to_categorical(batch_y)
        ...
        if epoch % save_step == 0 or epoch == training_epochs - 1:
            ...
            encode_decode = sess.run(X_decoded,
                                     feed_dict={X: batch_x[:examples_to_show]})#mnist.train.images[:examples_to_show]})
            ...
            for i in range(examples_to_show):
                ...
                #                cmap='gray',
                #                interpolation='none')
                a[0][i].imshow(batch_x[i].reshape(input_height, input_width),
                                cmap='gray',
                                interpolation='none')
